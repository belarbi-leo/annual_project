...
import com.pa2aresgi.pa2a.modelMapper.subClassImpl.UsersMapperSubClassImpl;
...
public class UsersServiceDTOImpl implements UsersServiceDTO {
    ...
    public List<UsersDTORead> readAll() {
        return usersRepository.findAll().stream().map(usersMapper::toDtoRead).collect(Collectors.toList());
    }

    @Override
    public List<UsersDTORead> readAll(Sort sort) {
        return usersRepository.findAll(sort).stream().map(usersMapper::toDtoRead).collect(Collectors.toList());
    }
    ...
    public List<UsersDTORead> readAllByRole(RoleEnum role){
        return usersRepository.findAllByRole(role).stream().map(usersMapper::toDtoRead).collect(Collectors.toList());
    }

    @Override
    public List<UsersDTORead> readAllByRole(RoleEnum role, Sort sort){
        return usersRepository.findAllByRole(role,sort).stream().map(usersMapper::toDtoRead).collect(Collectors.toList());
    }
