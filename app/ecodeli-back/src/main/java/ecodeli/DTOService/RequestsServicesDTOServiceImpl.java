...
public class RequestsServicesDTOServiceImpl implements RequestsServicesDTOService {
    ...
    @Override
    public List<RequestsServicesDTORead> readAllByUserReqIdUser(Integer userReqId){
        return requestsServicesRepository.findAllByUserReqIdUser(userReqId).stream().map(requestsServicesMapper::toDtoRead).toList();
    }
