...
type DropdownProps = {
  ...
  closeOnOutsideClick?: boolean; // Nouvelle prop pour contrôler la fermeture du dropdown
};
...
const Dropdown = ({
  ...
  menuClassName = "",
  optionClassName = "",
  closeOnOutsideClick = true, // Par défaut, ferme le dropdown lorsque l'on clique en dehors
}: DropdownProps) => {
  ...
  return (
    <div className="relative w-full mb-4" ref={dropdownRef}>
      ...
      >
        {renderButtonContent ? renderButtonContent() : options.find((option) => option.id === selected)?.name || label}
        {showChevron && <ChevronDownIcon className="w-5 h-5" />}
      </button>
      {isOpen && (
        <div className={clsx("absolute z-50 right-0 mt-2 w-full min-w-max border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-lg rounded-md overflow-hidden", menuClassName)}>
          {options.map((option) => (
            <button
              key={option.id}
              onClick={() => {
